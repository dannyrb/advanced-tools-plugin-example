(function() {
  ...
  function paintRegion(data, segmentationIndex, polygon) {
    if (!data.segmentations[segmentationIndex]) {
      ...
      seg.regions = seg.regions.filter(
        r => r && PolygonTools.polygon.is_ccw(r)
      );
    }
  }
  ...
  function imageRenderedCallback(e) {
    ...
    const data = cornerstoneTools.getToolState(e.detail.element, TOOL_NAME)
      .data[0];
    ...
    data.segmentations.forEach((seg, index) => {
      ...
      if (paths.length <= index) {
        ...
        pathElement.setAttributeNS(
          null,
          "fill",
          config.segmentationColors[index]
        );
  ...
  function dragCallback(e) {
    ...
    if (config.pinchToZoom && e.pointerType === "touch") {
      ...
      if (touchEventCache.size === 2) {
        ...
        const lastPoint1 = {
          x: e1.offsetX - (e.pointerId === e1.pointerId ? e1.movementX : 0),
          y: e1.offsetY - (e.pointerId === e1.pointerId ? e1.movementY : 0)
        };
        const lastPoint2 = {
          x: e2.offsetX - (e.pointerId === e2.pointerId ? e2.movementX : 0),
          y: e2.offsetY - (e.pointerId === e2.pointerId ? e2.movementY : 0)
        };
  ...
  function mouseWheelCallback(e) {
    ...
    if (e.shiftKey) {
      ...
      const config = svgBrush.getConfiguration();
      const newRadius =
        config.radius + e.detail.direction * Math.ceil(config.radius / 20);
  ...
  const svgBrush = mouseButtonTool(
    pointerDownCallback,
    mouseWheelCallback,
    activateCallback,
    deactivateCallback,
